# AWS AI Agent エンタープライズアーキテクチャクイックリファレンス

> AI エージェント開発に必要なコマンド、設定、およびベストプラクティス

---

## Bedrock Agent CLI

```bash
# 利用可能な基盤モデルの一覧表示
aws bedrock list-foundation-models --region us-east-1

# モデルの呼び出し
aws bedrock-runtime invoke-model \
    --model-id anthropic.claude-3-sonnet-20240229-v1:0 \
    --body '{"prompt": "Human: Hello\nAssistant:", "max_tokens_to_sample": 300}' \
    --cli-binary-format raw-in-base64-out \
    response.json

# Agent の作成
aws bedrock-agent create-agent \
    --agent-name my-enterprise-agent \
    --description "Enterprise AI Agent" \
    --foundation-model anthropic.claude-3-sonnet-20240229-v1:0 \
    --idle-session-ttl-in-seconds 1800 \
    --instruction "You are a helpful enterprise assistant..."

# Agent のデプロイ
aws bedrock-agent create-agent-alias \
    --agent-id <agent-id> \
    --agent-alias-name prod \
    --description "Production alias"

# Agent の呼び出し
aws bedrock-agent-runtime invoke-agent \
    --agent-id <agent-id> \
    --agent-alias-id <alias-id> \
    --session-id my-session-123 \
    --input-text "Analyze this data"
    --enable-trace
```

---

## トークンコスト計算

| モデル | Input ($/1K) | Output ($/1K) | 備考 |
|------|-------------|--------------|------|
| Claude 3 Opus | $0.015 | $0.075 | 最高品質 |
| Claude 3 Sonnet | $0.003 | $0.015 | バランス型の選択肢 |
| Claude 3 Haiku | $0.00025 | $0.00125 | 高速かつ経済的 |
| Titan Express | $0.0008 | $0.0016 | AWS 独自のモデル |

```python
# コスト見積もり関数
def estimate_cost(input_tokens, output_tokens, model='claude-3-sonnet'):
    pricing = {
        'claude-3-opus': {'input': 0.015, 'output': 0.075},
        'claude-3-sonnet': {'input': 0.003, 'output': 0.015},
        'claude-3-haiku': {'input': 0.00025, 'output': 0.00125}
    }
    
    p = pricing[model]
    input_cost = (input_tokens / 1000) * p['input']
    output_cost = (output_tokens / 1000) * p['output']
    
    return input_cost + output_cost
```

---

## セキュリティ設定チェックリスト

### プロンプトインジェクション対策
```python
# 簡易検出
INJECTION_PATTERNS = [
    r'ignore\s+previous',
    r'forget\s+instructions',
    r'system\s*:',
    r'you\s+are\s+now',
]
```

### IAM 最小権限
```json
{
  "Version": "2012-10-17",
  "Statement": [
    {
      "Effect": "Allow",
      "Action": [
        "bedrock:InvokeModel",
        "bedrock:InvokeAgent"
      ],
      "Resource": "arn:aws:bedrock:*::foundation-model/*",
      "Condition": {
        "StringEquals": {
          "aws:RequestedRegion": "us-east-1"
        }
      }
    }
  ]
}
```

---

## Docker 開発環境

```bash
# エージェントイメージのビルド
docker build -t ai-agent:dev .

# LocalStack 付きの開発環境を実行
docker-compose -f docker-compose.dev.yml up

# テストの実行
docker run --rm ai-agent:dev pytest tests/

# ローカル呼び出しテスト
curl -X POST http://localhost:8000/invoke \
  -H "Content-Type: application/json" \
  -d '{"input": "test", "session_id": "123"}'
```

---

## 監視指標

```python
# 主要な指標
METRICS = {
    'Latency': 'P99 < 2s',
    'TokenUsage': 'Daily budget tracking',
    'SuccessRate': '> 99%',
    'CostPerRequest': '< $0.01',
    'HumanApprovalRate': '< 5%'
}
```

---

## トラブルシューティング

| 症状 | 原因の可能性 | 解決策 |
|------|---------|----------|
| 高レイテンシ | トークンが多すぎる | チャンク処理を行う |
| コストの急増 | ループ呼び出し | 反復回数を制限する |
| 出力の不整合 | Temperature が高すぎる | 0.1-0.3 に設定する |
| 権限エラー | IAM 設定 | ポリシーを確認する |

---

## 参考リンク

- [Amazon Bedrock Docs](https://docs.aws.amazon.com/bedrock/)
- [AWS AI Service Security](https://docs.aws.amazon.com/security/)
- [Responsible AI Guide](https://aws.amazon.com/machine-learning/responsible-ai/)
