...
class ErrorType(Enum):
    ...
    CONTEXT_MISS = "context_miss"             # 上下文遗漏
    INSTRUCTION_MISS = "instruction_miss"     # 指令遗漏
    FORMAT_ERROR = "format_error"             # 格式错误
    AMBIGUITY = "ambiguity"                   # 歧义处理不当
...
Provide only the optimized prompt text without explanations."""
    ...
    def _evaluate_prompt(self, prompt: str, 
        ...
        for case in test_cases[:10]:  # 限制测试数量
            # 使用提示词生成响应
...
class ContinuousLearningPipeline:
    ...
    def trigger_retraining_check(self, threshold: float = 0.3) -> bool:
        ...
        if avg_rating < (5 * (1 - threshold)):  # 评分下降超过阈值
